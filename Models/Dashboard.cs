...
namespace DashboardApp.Models
{
    ...
    public class Dashboard : DbConnection
    {
        ...
        private void GetProductAnalisys()
        {
            ...
            using (var connection = GetConnection())
            {
                ...
                using (var command = new SqlCommand())
                {
                    ...
                    // Get Understock and send email alert if needed
                    command.CommandText = @"SELECT ProductName, Stock
                                        FROM Product
                                        WHERE Stock <= 6 AND IsDiscontinued = 0";
                    reader = command.ExecuteReader();
                    while (reader.Read())
                    {
                        var productName = reader[0].ToString();
                        var stock = (int)reader[1];

                        UnderstockList.Add(new KeyValuePair<string, int>(productName, stock));
        ...
        private async void SendLowStockAlert(string productName, int stock)
        {
            var toEmail = "davidgalolo56@gmail.com";
            var subject = "Low Stock Alert";
            var body = $"The stock for {productName} is low. Current stock: {stock}. Please restock.";
